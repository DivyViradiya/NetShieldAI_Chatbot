...
def parse_cloud_report(raw_text: str, chunk_size: int = 250) -> Dict[str, Any]:
    """
    Parses raw text from any report into structured, word-based chunks.

    Args:
        raw_text (str): The raw text content of any report.
        chunk_size (int): The number of words to include in each chunk.

    Returns:
        dict: A structured dictionary containing the text broken into chunks.
    """
    ...
    # 3. Group words into chunks of the specified size.
    content_chunks = []
...
def process_cloud_report_file(file_path: str, chunk_size: int = 250) -> Dict[str, Any]:
    """
    ...
    print(f"Processing cloud report: {file_path}")
    ...
    try:
        ...
        report_data = parse_cloud_report(raw_text, chunk_size)
        ...
        print(f"Error processing cloud report {file_path}: {str(e)}")
        raise
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2 or len(sys.argv) > 3:
        print("Usage: python cloud_parser.py <path_to_report.pdf> [chunk_size]")
        print("Example: python cloud_parser.py my_report.pdf 300")
        sys.exit(1)
    ...
    try:
        parsed_data = process_cloud_report_file(file_path, chunk_size)
        
        if parsed_data:
            print(f"\n--- Parsed Cloud Report (Chunk Size: {chunk_size}) ---")
